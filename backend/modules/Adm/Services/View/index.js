...
const CE = require('edge.js/src/Exceptions');
...
class Loader {
  constructor(viewsPath, presentersPath) {
    this._viewsPath = [{ path: viewsPath }];
  ...
  addViewsPath(_path, scope = '') {
    this._viewsPath.push({ path: _path, scope });
  }
  ...
  getViewPath(view) {
    ...
    return this.viewsPath.map((v) => {
      if (view.startsWith(v.scope)) {
        return path.join(v.path, view.replace(v.scope, ''));
      }
      return path.join(v.path, view);
    });
  }
  ...
  load(view) {
    ...
    const paths = this.getViewPath(view);

    for (let i = 0; i < paths.length; i += 1) {
      if (fs.existsSync(paths[i])) {
        try {
          return fs.readFileSync(paths[i], 'utf-8');
        } catch (error) {
          if (error.code === 'ENOENT') {
            throw CE.RuntimeException.missingView(view, paths[i]);
          }
          throw error;
        }
      }
