  ...
  init(...args) {
    this.table = $(this.tableSelector).DataTable({
      ...
      columns: [
        ...
        {
          data: 'updated_at', name: 'users.updated_at', orderable: true, searchable: false,
          render: (data) => {
            if (!data) return '';
            const localTime = moment.utc(data).local();
            return `<div class="font-weight-bolder text-primary
...

        {
          ...
          render: (data, type, row, meta) => {
            ...
            if (permissions.includes('users_edit')) {
              actions.push(`
                <li class="nav-item">
                  <a class="nav-link ${ADM.noHistoryClass}" href="${ADM.managerPath}/users/edit/${row.id}">
            ...
            if (permissions.includes('users_delete')) {
              actions.push(`
                <li class="nav-item">
                  <a class="nav-link ${ADM.noHistoryClass}" href="${ADM.managerPath}/users/delete/${row.id}"
                   data-confirm="${__('Adm.admin.remove')} - ${row.id}?" data-callback="users::ReloadTable">
