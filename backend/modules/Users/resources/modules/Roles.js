...
ADM.modules.set('roles', {
  ...
  init(...args) {
    this.table = $(this.tableSelector).DataTable({
      ...
      ajax: {
        url: `${ADM.managerPath}/users/roles/list`,
        type: 'POST',
        dataType: 'json',
      },
      ...
      columns: [
        ...
        {
          data: 'created_at', name: 'roles.created_at', orderable: true, searchable: false,
          render: (data) => {
            if (!data) return '';
        ...
        {
          data: 'updated_at', name: 'roles.updated_at', orderable: true, searchable: false,
          render: (data) => {
            if (!data) return '';
            const localTime = moment.utc(data).local();
            return `<div class="font-weight-bolder text-primary
...

        {
          ...
          render: (data, type, row, meta) => {
            ...
            if (permissions.includes('roles_edit')) {
              actions.push(`
                <li class="nav-item">
                  <a class="nav-link ${ADM.noHistoryClass}" href="${ADM.managerPath}/users/roles/edit/${row.id}">
            ...
            if (permissions.includes('roles_delete')) {
              actions.push(`
                <li class="nav-item">
                  <a class="nav-link ${ADM.noHistoryClass}" href="${ADM.managerPath}/users/roles/delete/${row.id}"
                   data-confirm="${__('Adm.admin.remove')} - ${row.id}?" data-callback="roles::ReloadTable">
