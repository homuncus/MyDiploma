...
ADM.modules.set('reports', {
  ...
  init(...args) {
    this.table = $(this.tableSelector).DataTable({
      ...
      ajax: {
        url: `${ADM.managerPath}/reports/list`,
        type: 'POST',
        dataType: 'json',
      },
      ...
      columns: [
        ...
        { data: 'user_name', name: 'reports.user_id', orderable: true, searchable: true },
        { data: 'description', name: 'reports.description', orderable: true, searchable: true },
        { data: 'solved', name: 'reports.solved', orderable: true, searchable: true },
        {
          data: 'created_at', name: 'reports.created_at', orderable: true, searchable: false,
          render: (data) => {
            if (!data) return '';
        ...
        {
          data: 'updated_at', name: 'reports.updated_at', orderable: true, searchable: false,
          render: (data) => {
            if (!data) return '';
            const localTime = moment.utc(data).local();
            return `<div class="font-weight-bolder text-primary
...

        {
          ...
          render: (data, type, row, meta) => {
            const actions = [];
            if (permissions.includes('reports_edit')) {
              actions.push(`
                <li class="nav-item">
                  <a class="nav-link ${ADM.noHistoryClass}" href="${ADM.managerPath}/reports/edit/${row.id}">
            ...
            if (permissions.includes('reports_delete')) {
              actions.push(`
                <li class="nav-item">
                  <a class="nav-link ${ADM.noHistoryClass}" href="${ADM.managerPath}/reports/delete/${row.id}"
                   data-confirm="${__('Adm.admin.remove')} - ${row.id}?" data-callback="reports::ReloadTable">
